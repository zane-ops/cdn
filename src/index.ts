...
function parseQueryParam<T extends string>(
    ...
    if (
        value &&
        (!allowedValues || (allowedValues as readonly string[]).includes(value))
    ) {
        return value as T;
    }
    return defaultValue;
        ...
        switch (url.pathname) {
            ...
            case "/api/single-release": {
                ...
                const releaseResponse = await fetch(
                    `https://api.github.com/repos/zane-ops/zane-ops/releases/tags/${tagName}`,
                    {
                        method: "GET",
                        headers: {
            ...
            case "/api/ping": {
                ...
                if (ip) {
                    ...
                    if (shouldRecordPing) {
                        // Insert ping record
                        await env.DB.prepare(
                            "INSERT INTO ip_pings (ip_hmac) VALUES (?);"
                        )
                            .bind(ipHmac)
                            .run();
                    ...
                    return Response.json({
                        success: false,
                        message:
                            "Ping not recorded - less than 30 minutes since last ping",
                    } as PingResponse);
                }
            ...
            case "/api/stats/summary": {
                ...
                const responseData = {
                    ...
                    totalUniqueUsers:
                        totalUniqueUsersResult?.totalUniqueUsers ?? 0,
                ...
                return addCors(
                    Response.json(responseData as SummaryStatsResponse)
                );
            }
            case "/api/pings/grouped": {
                ...
                try {
                    const { results } = await env.DB.prepare(sqlQuery)
                        ...
                        .all<GroupedPing>();
                    return addCors(
                        Response.json((results ?? []) as GroupedPingsResponse)
                    );
                } catch (e: any) {
            ...
            case "/api/pings/unique-activity": {
                ...
                try {
                    const dataStatement = env.DB.prepare(dataQuerySQL).bind(
                        pageSize,
                        offset
                    );
